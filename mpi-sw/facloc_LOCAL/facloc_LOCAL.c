		...
		const double *cost_close_open, const double *cost_secclose_open)
{
  ...
  for (b = 0; b < cities; b++) {
    ...
    if (change[b] != close[b]) {
      cout << "Suboptimal assignment of city " << b << ", " << change[b] << " instead of " << close[b] << endl;
      for (i = 1; i <= queues[0]; i++)
	cout << i << ": " << queues[b*facilities+i] << ", " << opened[queues[b*facilities+i]] << ", " 
